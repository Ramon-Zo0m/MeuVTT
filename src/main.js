...
// ESTADO DO JOGO
let gameState = {
    sceneImage: null, // Pode ser Imagem ou Vídeo
    tokens: [],
    playersLocked: false,
    currentAudio: null // Música tocando agora
};
...
function iniciarServidor() {
    ...
    io.on('connection', (socket) => {
        ...
        // --- SISTEMA DE IMAGEM/VÍDEO ---
        socket.on('request_variation', (data) => {
            // ... (Lógica de variação mantida igual)
...
ipcMain.handle('get-files', async (event, folder) => {
    ...
    // FILTRO QUE ACEITA VÍDEO E ÁUDIO
        ...
        if (fs.existsSync(caminho)) {
            ...
            if(io) {
                ...
                setTimeout(() => {
                    ...
                    if(gameState.currentAudio) io.emit('play_audio', gameState.currentAudio);
                }, 100);
            }
            return { success: true };
        }
